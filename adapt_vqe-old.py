        ...
        for i in range(1, max_cycle+1):
            ...
            if check_grad and norm <= gradient_norm_threshold:
                ...
                if print_details:
                    ...
                    print(F'Number of Parameters in wfn = {len(self.energy)}')
                    print(F'Number of calls to the energy function = {sum(self.num_queries)}')
                    print('\n--- reference ---')
                ...
                params = result.x
                self.num_queries.append(result.nit)
